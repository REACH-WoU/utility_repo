...
recode.label.to.value <- function(data, variables, labels, dictionary, issue, affect_na = FALSE){
  #' Recode a question by setting variables to some new value if they are matching a regex pattern.
  #' 
  #' @param data Dataframe containing records which will be affected.
  #' @param variables Vector of strings (or a single string) containing the names of the variables.
  #' @param labels The list of labels to be replaced by variables.
  #' @param issue String with explanation used for the cleaning log entry.
  ...
  clog <- tibble()
  for(variable in variables){
    if(affect_na) data1 <- data %>% filter((!!sym(variable) %in% labels) | is.na(!!sym(variable)))
    else data1 <- data %>% filter(!!sym(variable) %in% labels)
    if (length(data1$uuid)>0) {
      data1 <- data1 %>% mutate(variable = variable, old.value = !!sym(variable)) %>% select(any_of(CL_COLS))
      data2 <- data1[c("uuid","old.value")] %>% 
        rename("new.value" = "old.value") %>% 
        matchmaker::match_df(dictionary = dictionary, from = "from",               
                             to = "to",                   
                             by = "col")
      data2$issue <- issue
      cl <- left_join(data1,data2,by = "uuid")
...
create.translate.requests <- function(questions.db, responses.j, response_colname = "response.en", is.loop = F){
  ...
  #' @param is.loop unused
    ...
    relevant_colnames <- c("uuid", "name","loop_index", "ref.name","full.label","ref.type","ref.response", "choices.label", "today")
